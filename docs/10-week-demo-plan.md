...
| --- | --- | --- | --- |
| 1 — Decisions and running foundation | 30 / 10 | 6h close wireframe/policy questions and freeze baseline; 10h Nest scaffold + Alembic/DB/CI; 8h identity/deployment spike; 6h seed schema, scoped accounts and UI tokens. F06/F33. | Restricted URL serves a database-backed record. Login works for two accounts; API rejects an uninvited actor. Migration applies to empty DB. Normal-host production build succeeds or its blocker is fixed before feature work. |
| 2 — First persistent user journey | 34 / 6 | 8h scope policies/role navigation; 14h draft creation/save/resume and My Submissions; 6h profile/session/recovery; 6h DB/permission tests and seed reset. F06/F08/F09/F25. | Tester A creates draft, reloads and resumes; tester B cannot access it. Server restart retains it. Stale tab gets conflict. **Re-estimate all remaining work.** |
| 3 — Complete single submission | 36 / 4 | 14h all wizard variants/goals/terms; 10h bounded files and gallery association; 8h revision/snapshot/submit validation; 4h error/edge checks. F08–F12. | Five types can be saved and submitted with exact fields; valid sample upload succeeds, oversized/private access fails; reviewer sees immutable submitted data. |
| 4 — Review to visible publication | 36 / 4 | 14h shared technical/legal queues and decision UI; 10h transactional review/return/resubmit/publication; 8h catalog/detail API integration; 4h state/permission regression. F02/F03/F12–F14. | Three distinct actors complete draft → technical → legal → published. Returned version can be corrected. New catalog ID opens without rebuild; duplicate or stale decision cannot republish. |
| 5 — Access and communication | 32 / 8 | 12h sample acceptance/download gate; 12h inquiry/inbox/replies/notifications; 4h gallery/related entries/view events; 4h cross-user journey tests. F03–F05/F15. | Another tester finds publication, downloads sample and exchanges messages; unrelated user denied thread/private file. Withdrawn entry gets no new download grant. Core demo rehearsed with sponsor. |
| 6 — Organization and program use | 36 / 4 | 12h scoped overview/submissions/team UI; 10h membership/program commands; 8h org/program settings; 6h isolation/last-admin tests. F16–F20. | Org admin creates program and assigns existing identity; program admin cannot access sibling program. Pending membership and role edits persist without privilege escalation. |
| 7 — Governance and reporting | 34 / 6 | 12h super-admin org/user management; 8h moderation/override/audit; 8h exact overview aggregates/settings; 6h invalid-action and concurrency tests. F21–F24/F26. | All six roles usable; admin override has reason/history; takedown notifies; metrics reconcile to SQL records; defaults do not rewrite previous agreements. |
| 8 — Finish bounded breadth | 30 / 10 | 10h fixed CSV import/template/error flow; 14h existing public/content pages, resource assets, contact/interest, profile polish; 6h whole-app consistency and feedback fixes. F01/F07/F25/F27/F30–F32. | Valid CSV produces drafts once, invalid CSV none; retained visible links/actions work. **Feature freeze at week's end.** No guest/large-import work begins opportunistically. |
| 9 — Six-user rehearsal | 28 / 12 | 10h unscripted tester sessions and defect triage; 8h browser/keyboard/mobile checks; 6h restore/reset/rollback and six-user concurrency; 4h fixes/retest. All retained F IDs. | Complete core journeys; no unresolved cross-scope access, state corruption or data-loss defects. Seed reset and restore rehearsed. |
| 10 — Stabilize and hand over | 24 / 16 | 12h prioritized defects; 6h final seed/demo script and release verification; 6h runbooks, architecture handover and phase-2 backlog. | Stable release accepted by sponsor; credentials privately distributed; operator can deploy/reset/restore. Remaining cuts/limitations visible. |
| **Total** | **320 / 80** | **400 hours** | |
