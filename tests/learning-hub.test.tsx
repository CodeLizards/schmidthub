...
import LicensingTopicPage from "@/app/topics/licensing/page";
...
describe("IP Learning Hub", () => {
  it("links the topic cards and six sample resources", () => {
    ...
    expect(screen.getByRole("link", { name: /Licensing options/ })).toHaveAttribute("href", "/topics/licensing");
  ...
  it("renders all five licensing options at the topic route", () => {
    render(<LicensingTopicPage />);
    expect(screen.getByRole("heading", { level: 1, name: "Licensing options" })).toBeVisible();
    expect(within(screen.getByRole("article")).getAllByRole("heading", { level: 2 })).toHaveLength(5);
    expect(screen.getByRole("link", { name: /Back to the IP Learning Hub/ })).toHaveAttribute("href", "/learning-hub");
  });
  ...
  it("selects the section of a linked glossary term", async () => {
    window.history.replaceState(null, "", "#license");
    try {
      render(<GlossaryPage />);
      await waitFor(() => expect(screen.getByRole("link", { name: "Licensing" })).toHaveAttribute("aria-current", "location"));
    } finally {
