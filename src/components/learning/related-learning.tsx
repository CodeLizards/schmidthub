...
export function RelatedLearning({ topic, excludeSlug }: { topic?: "ip" | "licensing"; excludeSlug?: string }) {
  const topicLink = topic === "ip"
    ? { href: "/licensing", title: "Licensing options", description: "The five licenses SHIP supports, from fully open to negotiated commercial terms." }
  ...
  return <section className="max-w-4xl mx-auto px-6 py-14 border-t border-border" aria-label="Relevant topics">
    <div className="flex flex-wrap items-center justify-between gap-4 mb-6"><h2 className="font-serif text-2xl">Relevant topics</h2><Link href="/learning-hub" className="text-sm underline underline-offset-4 text-muted hover:text-ink">Visit the IP Learning Hub →</Link></div>
